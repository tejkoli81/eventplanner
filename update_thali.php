...
if (isset($_GET['id'])) {
    $query = 'SELECT * FROM thali WHERE t_id = ' . $_GET['id'];
...
<?php if (isset($_POST['save'])) {
    ...
    mysqli_query(
        $con,
        "update thali set t_name='$name', t_type='$type', t_rate=$rate, t_img='$path' where t_id=$tid"
    );
    ?>
	<script>
	    swal({title: "Thali", text: "Thali updated successfully", type: "success"}, function(){location.href='manage_thali.php'});
	</script>			
<?php
} ?>
								...
								<div class="row">
									<form method="POST" class="w-100 p-3" enctype="multipart/form-data">
                                        <div class="form-group">
									    	<label for="tid" style="font-weight: 600;">Thali ID</label>
									    	<input type="text" name="tid" class="form-control shadow-none" id="id" readOnly value="<?= $data[
                  't_id'
              ] ?>">
									  	</div>
									  	<div class="form-group">
									    	<label for="name" style="font-weight: 600;">Thali Name</label>
									    	<input type="text" name="name" class="form-control shadow-none" id="name" value="<?= $data[
                  't_name'
              ] ?>" required>
									  	</div>
									  	<div class="form-group">
                                            ...
                                            <option value="Veg" <?php if (
                                                $data['t_type'] == 'Veg'
                                            ) {
                                                echo ' selected';
                                            } ?>>Veg</option>
                                            <option value="Non-veg" <?php if (
                                                $data['t_type'] == 'Non-veg'
                                            ) {
                                                echo ' selected';
                                            } ?>>Non-veg</option>
									  	...
									  	<div class="form-group">
									    	...
									    	<input type="number" name="rate" class="form-control shadow-none" id="rate" min=0 value="<?= $data[
                  't_rate'
              ] ?>" required>
									  	</div>
									  	<div class="form-group">
									    	<label for="photo" style="font-weight: 600;">Thali Photo</label><br>
                                            <a href="<?= $data[
                                                't_img'
                                            ] ?>">View</a>
