...
<?php if (isset($_POST['update'])) {
    ...
    $query = 'SELECT * from customer WHERE cust_id = ' . $_SESSION['cid'];
    $result = mysqli_query($con, $query);

    if ($data = mysqli_fetch_assoc($result)) {
        if ($data['cust_pwd'] == $oldPassword) {
            if ($newPassword == $confirmPassword) {
                mysqli_query(
                    $con,
                    "UPDATE customer SET cust_pwd = '$newPassword' WHERE cust_id=" .
                        $_SESSION['cid']
                ); ?>
	<script>
	    swal({title: "Customer Settings", text: "Customer password updated successfully", type: "success"}, function(){location.href='cust-settings.php'});
	</script>			
<?php
            } else {
                 ?>
	<script>
	    swal({title: "Customer Settings", text: 'New password & confirm password not matching', type: "error"}, function(){location.href='cust-settings.php'});
	</script>			
<?php
            }
        } else {
             ?>
	<script>
	    swal({title: "Customer Settings", text: 'Current password is incorrect.', type: "error"}, function(){location.href='cust-settings.php'});
	</script>			
<?php
        }
    }
