...
<?php if (isset($_POST['update'])) {
    ...
    $query = 'SELECT * from admin WHERE admin_id = ' . $_SESSION['aid'];
    $result = mysqli_query($con, $query);

    if ($data = mysqli_fetch_assoc($result)) {
        if ($data['admin_pwd'] == $oldPassword) {
            if ($newPassword == $confirmPassword) {
                mysqli_query(
                    $con,
                    "UPDATE admin SET admin_pwd = '$newPassword' WHERE admin_id=" .
                        $_SESSION['aid']
                ); ?>
	<script>
	    swal({title: "Admin Settings", text: "Admin password updated successfully", type: "success"}, function(){location.href='admin-settings.php'});
	</script>			
<?php
            } else {
                 ?>
	<script>
	    swal({title: "Admin Settings", text: 'New password & confirm password not matching', type: "error"}, function(){location.href='admin-settings.php'});
	</script>			
<?php
            }
        } else {
             ?>
	<script>
	    swal({title: "Admin Settings", text: 'Current password is incorrect.', type: "error"}, function(){location.href='admin-settings.php'});
	</script>			
<?php
        }
    }
