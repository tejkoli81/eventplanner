						...
						<div class="below-toggle-content">
							<div class="col-md-12">
								<div class="top-part mb-3">
									<h2 class="d-inline">Welcome Vendor</h2>
									<h4 class="d-inline ml-2"><?= $_SESSION['vname'] ?></h4>
								</div>
							</div>
						</div>
					</div>
...
<?php
if ($_SESSION['type'] == 'Decorator') {
    $rs = mysqli_query(
        $con,
        'select booking_no,booking_date,event_date,event_name, venue_name, venue_phone, decorator_id, caterer_id, p_id, planner_name, planner_phone, t_name, no_of_person ' .
            'from booking,customer,venue,thali,planner ' .
            'where booking.cust_id = customer.cust_id and ' .
            'booking.t_id = thali.t_id and booking.venue_id = venue.venue_id and ' .
            "status='Bill Paid' and booking.planner_id = planner.planner_id and " .
            'booking.decorator_id=' .
            $_SESSION['vid']
    );
}
if ($_SESSION['type'] == 'Caterer') {
    $rs = mysqli_query(
        $con,
        'select booking_no,booking_date,event_date,event_name, venue_name, venue_phone, decorator_id, caterer_id, p_id, planner_name, planner_phone, t_name, no_of_person ' .
            'from booking,customer,venue,thali,planner ' .
...
<?php while ($row = mysqli_fetch_row($rs)) { ?>
	<tr>
        ...
        <td><?= $row[5] ?></td>
<?php
$rs1 = mysqli_query($con, 'select * from vendor where v_id=' . $row[6]);
$row1 = mysqli_fetch_assoc($rs1);
?>
		<td><?= $row1['v_name'] ?></td>
		<td><?= $row1['v_phone'] ?></td>
		<?php
  $rs1 = mysqli_query($con, 'select * from vendor where v_id=' . $row[7]);
  $row1 = mysqli_fetch_assoc($rs1);
  ?>
		<td><?= $row1['v_name'] ?></td>
		<td><?= $row1['v_phone'] ?></td>
