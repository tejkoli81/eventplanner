...
<?php if (isset($_POST['update'])) {
    ...
    $query = 'SELECT * from planner WHERE planner_id = ' . $_SESSION['pid'];
    $result = mysqli_query($con, $query);

    if ($data = mysqli_fetch_assoc($result)) {
        if ($data['planner_pwd'] == $oldPassword) {
            if ($newPassword == $confirmPassword) {
                mysqli_query(
                    $con,
                    "UPDATE planner SET planner_pwd = '$newPassword' WHERE planner_id=" .
                        $_SESSION['pid']
                ); ?>
	<script>
	    swal({title: "Planner Settings", text: "Planner password updated successfully", type: "success"}, function(){location.href='planner-settings.php'});
	</script>			
<?php
            } else {
                 ?>
	<script>
	    swal({title: "Planner Settings", text: 'New password & confirm password not matching', type: "error"}, function(){location.href='planner-settings.php'});
	</script>			
<?php
            }
        } else {
             ?>
	<script>
	    swal({title: "Planner Settings", text: 'Current password is incorrect.', type: "error"}, function(){location.href='planner-settings.php'});
	</script>			
<?php
        }
    }
						...
						<div class="below-toggle-content">
							<div class="col-md-12">
								<div class="top-part mb-3">
									<h2 class="d-inline">Planner</h2>
									<p class="d-inline ml-2">Settings</p>
									<a href="planner_home.php" class="d-inline text-dark mt-2" style="text-decoration: none; float: right; font-weight: 500;"><i class="fas fa-tachometer-alt"></i> Dashboard</a>
								</div>
								<div class="row">
